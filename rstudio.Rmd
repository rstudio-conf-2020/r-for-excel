...
- explore RMarkdown. This will be your notes.
...
Let's start of by looking at the 3 code chunks, which are grey. There are two things to look at: R code and code options
...
Each of them are start by 3 backticks and `{r LABEL}`, which signify that there will be R code inside. Anything inside the brackets (`{ }`) is instructions for RMarkdown about that code to run. For example:
...
- all three chunks say `r` as the language, and have a name (`setup`, `cars`, `pressure`. This is to help us navigate between them and keep them organized. They are optional.
