...
- inverts.xlsx
- fish.csv
- kelp.xlsx (just read in the "abur" worksheet by adding argument `sheet = "abur"` to `read_excel()`)
...
**Task**: Create a subset from the **fish** data frame, stored as object **fish_abur**, that only contains observations from Arroyo Burro (site entered as "abur"). 
...
**Task:** Create a subset from the **fish** data frame that only contains observations from year 2018, and store as object **fish_2018**.
...
**Task:** Create a subset from **fish** called **fish_gar_2016** that keeps all observations if the year is 2016 *OR* the species is "garibaldi."
...
Sometimes we'll only want to keep observations that **do** satisfy multiple conditions (e.g., to keep this observation it must satisfy this **AND** that condition). For example, we may want to create a subset that only returns rows from **inverts** where the **site** is "abur" or "mohk" *and* the **common_name** is "purple urchin." 
...
Check it out to see that only observations where the site is "abur" or "mohk" **AND** the common name is "purple urchin" are retained: 
```{r}
urchin_abur_mohk
```
...
Take a look to ensure that only those years are included with `View(fish_16_17)`. Now, let's merge it with our Arroyo Burro kelp fronds data (kelp_abur) in different ways. 
...
Explore the data frame. Is that what we want? Probably not...we might want to update our year format in kelp_abur to a 4-digit year instead. Let's do that by adding 20 to the existing values, then trying again! 

```{r}
kelp_abur_new <- kelp_abur %>% 
...
Now, we want to keep all observations in *fish_16_17*, and merge them with *kelp_abur* while only keeping observations from *kelp_abur* that match an observation within *fish_16_17*. So when we use `left_join()`, any information on kelp frond counts from 2018 should be dropped, because those wouldn't have a match in the left data frame. 
