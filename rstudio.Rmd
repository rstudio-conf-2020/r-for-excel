...
OK, first off: by opening a file, we are seeing the 4th pane of the RStudio console, which here is a text editor. This lets us organize our files within RStudio instead of having a bunch of different windows open.
...
Introduce yourself to the person sitting next to you. Discuss what you notice with these two files. Share-out with the group. (5 mins)
...
Notice how the word `FALSE` is all capitals. Capitalization matters in R; `TRUE/FALSE` is something that R can interpret as a binary yes/no or 1/0. This is different from true/false or True/False. 
...
Now, let's write some code in R. Let's say we want to see the summary of the `pressure` data. I'm going to press enter to to add some extra carriage returns because sometimes I find it more pleasant to look at my code, and it helps in troubleshooing, which is often about identifying typos. R lets you use as much whitespace as you would like.
...
It is important to be able to run R code directly rather than only executing it when you knit the document. We can write code in the Console to interact with the live R process.
...
We can also do math as we can in Excel: 
...
Error messages are R's way of saying that it didn't understand what you said. This is like in English when we say "What?" or "Pardon?" And like in spoken language, some error messages are more helpful than others. Like if someone says "Sorry, I didn't understand that last word" rather than only "What?"

In this case, R is saying "I didn't understand `pressur`". R tracks the datasets it has out-of-the box as objects, as well as any additional objects that you make and `pressur` is not among them, so it says that it is not found. 

Errors will be less frustrating if you have the understanding and mindset that **most likely the problem is your typo or misuse**, and not that R is broken or hates you. 

The first step of becoming a proficent R user is to move past the exasperation of "it's not working!" and **read the error message**. 
...
First: This error message starts out in a pretty cryptic way: I don't expect you to know what `parse_block(g[-1]...` means. But, assuming that the error message is really trying to help me, I continue scanning the message which allows me to identify the problem: `duplicate label 'cars'`. 
...
So far we have written code in our RMarkdown file and knitted the file, and we have also separately written code in the Console. But we can also send R code that we've written in an RMarkdown chunk (the grey R code) down into the Console in RStudio. 

How do we do it? There are several ways. Let's do each of these with `summary(pressure)`.

1. copy-paste the `summary(pressure)` code into the console.
   a. important!!: I am only highlighting `summary(pressure)`, not any of the backticks/fences! If you see the error ````{r}`Error: attempt to use zero-length variable name` it is because you have accidentally highlighted the backticks along with the R code. Try again (and don't forget that you can add spaces within the code chunk or make your RStudio session bigger (View > Zoom In)
1. run selected lines by highlighting — or simply putting the cursor on the line — and clicking 'Run'. This is available from: 
    a. the bar above the file (green arrow)
    b. the menu bar: Code > Run Selected Line(s)
    c. keyboard shortcut: command-return
1. run the whole chunk by clicking the little black down arrow next to the Run green arrow and selecting Run Current Chunk. Notice there are also options to run all chunks, run all chunks above or below...
...
Let's look up another function in the Help Pages. In Excel, there is a "SUM" function to calculate a total. If we try `?SUM` in R, before we hit enter, RStudio will offer suggestions about what we might mean. Scroll down...we don't mean "Summary" but eventually we find the function we want spelled with all lowercase letters: `?sum()`. Hit enter. 

From the Help pages you'll see that we can pass any arguments we want (...) and we also see that `na.rm = FALSE`. This is the same capital FALSE that we've seen before: it is a logical (yes/no 1/0) operator that says that by default, `sum()` will attempt to calculate the total without removing any missing values.
...
The cars data don't have any missing values, but to demonstrate how we could tell R to remove missing values before calculating: 

```{r sum-cars-na, eval=FALSE}
sum(cars$dist, na.rm = TRUE)
```
...
```{r c-ex2}
c("San Francisco", "Golden Gate Bridge", "Cal Academy") 
```
...
If we add a number as an argument, you'll see that R will respond by returning the number in quotes. It is no longer a numeric, it is a string. 

```{r c-ex3}
c(7, "San Francisco", "Golden Gate Bridge", "Cal Academy") 
```

This is a big difference between R and Excel, since Excel allows you to have a mix of text and numerics in the same column or row. R's way can feel restrictive, but TODO finish this thought (Shotwell blog)
...
In Excel, data are stored in the spreadsheet. In R, they are stored in objects. Data can be a variety of formats, for example numeric and text like we just talked about.  
...
We can also assign objects with existing objects. Let's say we want to have the distance traveled by cars in its own variable, and multiply by 1000 (assuming these data are in km and we want m). TODO better example?
...
TODO: add Horst Illustration / OR remove and let Allison do this?
...
You don't need to go to CRAN's website to install packages, this can be accomplished within R using the command `install.packages("package-name-in-quotes")`. How do you know what packages exist? Well, how do you know what movies exist on iTunes? You learn what's available based on your needs, interests the community around you. We'll introduce you to several really powerful packages that we work with and help you find others that might be of interest to you.
...
TODO: MAYBE AS SLIDES? // BUILD OUT FROM SYNTHESIS EXAMPLE
...
We will use the `usethis` package to configure **git** with global commands, which means it will apply 'globally' to all files on your computer, rather than to a specific folder. Our username and email will be the arguments we pass to the function `use_git_config`. <!---TODO smooth wording --->
...
TODO: should we have them fork and clone the book? or a test toy repo?


should this be from [Allison's googledoc](https://docs.google.com/document/d/1zx2upJJqFZe94O3BQSMI56Z76s3haLXC0otKSpcZaJQ/edit)
