...
## Getting started - Create a new .Rmd, attach packages
...
We're going to be doing a lot of plot variations with those same variables. Let's store the first line as object `ggbase` so that we don't need to retype it each time: 
...
TODO: do they have a figures folder created yet? 
```{r, eval = FALSE}
